...
float teamColorIntensity(const float3 pos, const float3 normal)
{
	return saturate(pos.y / 16.f);
}
...
void main(in VS_Input VSInput, out PS_Input PSInput) {
	...
	float intensity = teamColorIntensity(VSInput.position, normal);
	PSInput.teamColor = float4(TEAM_COLOR.rgb, intensity);
